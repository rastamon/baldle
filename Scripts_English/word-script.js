...
const WORD_LENGTH = 5;
const FLIP_ANIMATION_DURARTION = 500
const DANCE_ANIMATION_DURARTION = 500
const MIN_TO_MS = 60 * 1000
const keyboard = document.querySelector('[data-keyboard')
const alertContainer = document.querySelector('[data-alert-container')
const guessGrid = document.querySelector("[data-guess-grid]")
const recordContainer = document.querySelector("[data-record-container]")
const gameReset = document.querySelector("[data-game-reset]")
const screen = document.querySelector("[data-race]")
let winCounter = 0;
let lossCounter = 0;
let guessCounter = 0;
let roundCounter = 0;
...
let chosenRace = races[randomNumber(2)]
console.log(chosenRace)
const targetWord = baldWords[randomNumber(baldWords.length - 1)]

screen.dataset.race = chosenRace;
...
function handleKeyPress(e) {
    if (e.key === "Enter" && chosenRace == "black" && ((guessCounter - 1) % 2)) {
        submitGuess()
        setTimeout(() => {
            fillKeys()
        }, 1500);
...
function fillKeys() {
    fillWord.forEach(word => {
        activeTiles = getActiveTiles()
        const nextTile = guessGrid.querySelector(":not([data-letter])")
        nextTile.dataset.letter = word
        nextTile.textContent = word
        nextTile.dataset.state = "active"
...
function flipTile(tile, index, array, guess) {
    ...
    }, index * FLIP_ANIMATION_DURARTION / 2)
...
function setScore(scores) {
    const records = recordContainer.querySelectorAll('[data-score]')
    records[roundCounter].dataset.score = scores;
    if (scores == "win") {
        winCounter++
    } else if (scores == "lose") {
        lossCounter++
...
function newGame() {
    ...
    chosenRace = races[randomNumber(2)]
    target = baldWords[randomNumber(baldWords.length - 1)]
    screen.dataset.race = chosenRace;
    startInteraction()
    resetTimer()
    checkGameState()
